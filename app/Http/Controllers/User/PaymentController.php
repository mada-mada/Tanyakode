...
class PaymentController extends Controller
{
  ...
  {
    // Pastikan user sudah memverifikasi OTP / aktivasi
    if (Auth::check() && Auth::user()->status === 'verify') {
      return response()->json(['status' => 'error', 'message' => 'Silakan verifikasi akun Anda terlebih dahulu.'], 403);
    }
  ...
  {
    // Pastikan user sudah verifikasi
    if (Auth::check() && Auth::user()->status === 'verify') {
      return response()->json(['status' => 'error', 'message' => 'Silakan verifikasi akun Anda terlebih dahulu.'], 403);
    }
