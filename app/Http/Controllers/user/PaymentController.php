...
public function show($slug)
{
    ...
    // Ambil riwayat transaksi user untuk course ini
    $orders = Transaction::where('user_id', Auth::id())
                ->where('course_id', $course->id)
                ->orderBy('created_at', 'desc')
                ->get();

    // 2. Konfigurasi Midtrans (Tetap biarkan, tapi akan kita handle error-nya nanti)
    Config::$serverKey = config('services.midtrans.serverKey');
    ...
    // --- LOGIKA BARU YANG LEBIH AMAN (TRY-CATCH TANPA DIE) ---
    try {
        if ($transaction) {
            // Jika sudah ada transaksi pending, pakai token lama
            $snapToken = $transaction->snap_token;
        } else {
            // HANYA JIKA BELUM PERNAH BELI / STATUSNYA BUKAN PENDING, COBA BUAT BARU
            // TAPI, KARENA INI HANYA VIEW (SHOW), SEBAIKNYA JANGAN AUTO-CREATE TRANSAKSI DI SINI.
            // UNTUK SEMENTARA (AGAR TIDAK ERROR 401), KITA SKIP PEMBUATAN TOKEN OTOMATIS
            // BIARKAN USER KLIK TOMBOL 'BUAT PESANAN' DULU (NANTI KITA BUAT ROUTE TERPISAH).
            
            // NAMUN, UNTUK MENGIKUTI ALUR KAMU SAAT INI (AUTO CREATE), SAYA BERIKAN SAFE GUARD:
            ...
            // Coba request ke Midtrans
            // Jika gagal (karena Server Key dummy), dia akan masuk ke block catch
            $snapToken = Snap::getSnapToken($params);

            // Jika berhasil dapat token, baru simpan ke DB
        ...
        // --- PENANGANAN ERROR ---
        // Jangan dd() atau matikan aplikasi. 
        // Biarkan snapToken null, tapi catat errornya (opsional) atau abaikan saja agar tampilan tetap muncul.
        $snapToken = null; 
        
        // Opsional: Kirim pesan error ke view (jika ingin ditampilkan di alert)
        session()->flash('warning', 'Sistem pembayaran sedang dalam mode maintenance (Midtrans Key Invalid). Anda tetap bisa mengakses materi gratis.');
    }

    // 4. Kirim data ke View dengan Aman
