        ...
        <div class="container position-relative">
            <div class="row">
                <div class="col-lg-8">
                    <div class="mb-3">
                        <span
                            class="badge bg-white bg-opacity-10 border border-white border-opacity-20 text-white px-3 py-2 rounded-pill fw-bold">
                    ...
                    <div class="d-flex flex-wrap gap-4 text-sm text-white text-opacity-90 fw-medium">
                        <span class="me-3"><i class="fas fa-book-open me-2"></i>
                            {{ $course->modules ? $course->modules->count() : 0 }} Modul</span>
            ...
            <div class="col-lg-8 mb-5">
                ...
                <div class="mb-4">
                    <div class="d-flex justify-content-between align-items-end mb-3">
                        ...
                        <span class="text-muted small fw-bold">{{ $course->modules ? $course->modules->count() : 0 }} Modul
                            • {{ $totalContent }} Materi</span>
                    </div>
                                                ...
                                                <li class="d-flex align-items-center py-2 border-bottom last:border-0">
                                                    @if (Str::contains(strtolower($content->content_type ?? 'video'), 'video'))
                                                        <div class="bg-light rounded-circle p-2 me-3 text-primary"><i
                                                                class="fas fa-play fa-sm"></i></div>
                                                    @else
                                                        <div class="bg-light rounded-circle p-2 me-3 text-warning"><i
                                                                class="fas fa-file-alt fa-sm"></i></div>
                                                    @endif
                    ...
                    <div class="card-clean p-4">
                        ...
                        @auth
                            @if ($course->price > 0 && !auth()->user()->hasPurchased($course->id))
                                <div class="mb-3">
                                    <label class="form-label small font-weight-bold text-muted text-uppercase">Punya
                                        Voucher?</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control form-control-sm" id="voucher-code"
                                            placeholder="Kode Voucher">
                                        <button class="btn btn-dark btn-sm" type="button"
                                            id="apply-voucher-btn">Gunakan</button>
                                    </div>
                        ...
                        <div class="mb-4">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                ...
                                @else
                                    <span class="fw-bold text-dark" id="display-original-price">Rp
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                @else
                                    <span class="h4 text-primary font-weight-bold mb-0" id="display-final-price">Rp
                                        {{ number_format($course->price, 0, ',', '.') }}</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="d-grid gap-2 mb-4">
                            @auth
                                ...
                                @if (auth()->user()->hasPurchased($course->id))
                                    ...
                                    <a href="{{ route('user.courses.learning', ['slug' => $course->slug]) }}"
                                        class="btn btn-success btn-lg btn-block w-100 font-weight-bold"
                                    ...
                                    @if ($course->price > 0)
                                        ...
                                        @if (auth()->check() && auth()->user()->status === 'verify')
                                            <button class="btn btn-warning" disabled>
                                                Verifikasi dulu untuk bisa membeli <i
                                                    class="fas fa-exclamation-circle ms-2"></i>
                                            </button>
                                            <a href="{{ route('otp.verification') }}" class="btn btn-link">Verifikasi
                                                Sekarang</a>
                                        @else
                                            <button id="pay-button"
                                                class="btn-primary-custom text-center text-decoration-none">
                        ...
                        <div class="bg-light p-3 rounded-3">
                            ...
                            <ul class="list-unstyled d-flex flex-column gap-2 text-sm text-dark mb-0">
                                <li class="d-flex align-items-center text-muted small"><i
                                        class="fas fa-clock text-success me-2"></i> Akses Selamanya</li>
                                <li class="d-flex align-items-center text-muted small"><i
                                        class="fas fa-mobile-alt text-success me-2"></i> Akses HP & PC</li>
                                <li class="d-flex align-items-center text-muted small"><i
                                        class="fas fa-certificate text-success me-2"></i> Sertifikat Penyelesaian</li>
                            </ul>
                        </div>
                            ...
                            window.snap.pay(data.snap_token, {
                                onSuccess: function(result) {
                                    window.location.href =
                                        "{{ route('user.payment.success') }}?order_id=" +
                                        result.order_id + "&transaction_status=settlement";
                                },
                                onPending: function(result) {
                                    Swal.fire('Menunggu', 'Selesaikan pembayaran Anda.',
                                        'info');
                                },
                                ...
                                onClose: function() {
                                    Swal.fire('Dibatalkan', 'Anda menutup popup.',
                                        'warning');
                                }
                            });
                        }
