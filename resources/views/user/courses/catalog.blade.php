    ...
    <div class="container-fluid p-4">
        ...
        <div class="d-flex gap-2 mb-5 overflow-auto pb-2">
            <a href="{{ route('user.courses.catalog') }}"
                class="filter-btn {{ !request('level') || request('level') == 'Semua' ? 'active' : '' }}">Semua</a>
            <a href="{{ route('user.courses.catalog', ['level' => 'Pemula', 'search' => request('search')]) }}"
                class="filter-btn {{ request('level') == 'Pemula' ? 'active' : '' }}">Beginner</a>
            <a href="{{ route('user.courses.catalog', ['level' => 'Menengah', 'search' => request('search')]) }}"
                class="filter-btn {{ request('level') == 'Menengah' ? 'active' : '' }}">Intermediate</a>
            <a href="{{ route('user.courses.catalog', ['level' => 'Lanjutan', 'search' => request('search')]) }}"
                class="filter-btn {{ request('level') == 'Lanjutan' ? 'active' : '' }}">Expert</a>
        </div>
            ...
            @forelse($courses as $course)
                ...
                <div class="col-12 col-md-6 col-lg-4 mb-4">
                    <div class="card card-custom h-100">
                        ...
                        <div class="bg-dark-theme p-0 position-relative text-center d-flex justify-content-center align-items-center"
                            style="height: 220px; overflow: hidden;">

                            {{-- Badge Level (Beginner/Expert) --}}
                            <span class="badge {{ $levelConfig['class'] }} position-absolute shadow-sm"
                                style="top: 20px; right: 20px; padding: 8px 16px; border-radius: 20px; z-index: 10;">
                                {{ $levelConfig['label'] }}
                            </span>
                            ...
                            @if ($course->thumbnail_url && file_exists(public_path('storage/' . basename($course->thumbnail_url))))
                                <img src="{{ asset('storage/' . $course->thumbnail_url) }}" alt="{{ $courseName }}"
                            ...
                            @else
                                ...
                                <h1 class="text-cyan-theme fw-bold m-0 position-relative"
                                    style="font-size: 6rem; z-index: 5; opacity: 0.5;">
                                    {{ $initials }}
                                </h1>
                            @endif
                            ...
                            @if ($course->school_id)
                                <div class="position-absolute bottom-0 start-0 w-100 bg-warning text-dark py-1 small fw-bold"
                                    style="z-index: 11; opacity: 0.9;">
            ...
            @empty
                <div class="col-12">
                    <div class="alert alert-light shadow-sm border-0 text-center py-5">
                        ...
                        <a href="{{ route('user.courses.catalog') }}" class="btn btn-outline-secondary btn-sm mt-2">Reset
                            Filter</a>
                    </div>
                </div>
            @endforelse
